...
if __name__ == "__main__":
  ...
  parser.add_argument('--user', help = "Authentication with REST API: Username",
                      ...
                      dest = "user",
                      default="admin")
  parser.add_argument('--options', help = "The NFS export options to use (comma-separated)",
                      action = "store",
                      dest = "options",
                      default = "rw")
  ...
  results = jsonrequest("nfs_export_add", dict(client=client, pool=pool, name=volName, options=options))
  sys.exit(0)
