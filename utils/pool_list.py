...
if __name__ == "__main__":
  ...
  if pool is not None:
    results = jsonrequest("pool_list", dict(name=pool))
  else:
    results = jsonrequest("pool_list")

  print("%-6s %-25s %-25s %-25s" % ("Type", "Pool", "Size", "Free"))
  for result in results:
      print("%-6s %-25s %-25s %-25s" %
          (str(result['type']), str(result['name']), str(result['size']), str(result['free_size'])))
