...
if __name__ == "__main__":
  ...
  try:
    results = jsonrequest("filesys_create", dict(pool=pool, name=volName, size=volSize, client=client))
  except Exception as create_error:
    print("Problem executing filesys_create: %s" % create_error)
    sys.exit(1)

  print("Created fs volume - Pool:%s - Name:%s - Size:%s - Client:%s" % (pool, volName, volSize, client))
  sys.exit(0)
