...
if __name__ == "__main__":
  ...
  parser.add_argument('--all', help = "List out all exports regardless of initiator or pool",
                      action = "store_true",
                      dest = "list_all",
                      default = False)
  ...
  # If we aren't interested in restricting the list to a specific initiator
  # name, then don't bother reading the file.
  if initiatorName is None and not list_all:
  ...
  #XXX: Do we want to always list out all pools, or have multiple --all operations?
  if list_all:
    results = jsonrequest("export_list")
  else:
    results = jsonrequest("export_list", dict(pool=pool, initiator_wwn=initiatorName))
  space_formatting="%-25s %-15s %-40s %-3s %-10s %s"
  print(space_formatting % ('vol_name',
                            'vol_size',
                            'initiator_wwn',
                            'lun',
                            'pool',
  ...
  for result in results:
      print(space_formatting % (str(result['vol_name']),
                                str(result['vol_size']),
