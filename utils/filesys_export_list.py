...
if __name__ == "__main__":
  ...
  results = jsonrequest("nfs_export_list")

  space_formatting="%-16s %-50s %s"
  print(space_formatting % ('Client',
                            'Path',
                            'Options'))
  for result in results:
      print(space_formatting % (str(result['client']),
                                str(result['path']),
                                str(','.join(result['options']))))
